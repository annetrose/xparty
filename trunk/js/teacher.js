...
var DATA_PANES = { STUDENT_PANE:'Students', HISTORY_PANE:'Task History' };
...
function initUI() {
    ...
    for (paneKey in DATA_PANES) {
    	var paneLabel = DATA_PANES[paneKey];
    	var buttonId = getPaneButtonId(paneKey);
    	html += '<button class="load_btn cssbtn" id="'+buttonId+'" onclick="loadPane(\''+paneKey+'\')">'+paneLabel+'</button><br/>';
    }
...
function updateUI() {
    ...
    $('#'+getPaneButtonId(STUDENT_PANE)).html("Students (" + getStudentCount() + ")");
...
function loadStudentPane() {
	...
	var html = '<h3 id="pane_title" style="margin-bottom:10px">Students</h3>';
...
function loadTaskHistoryPane() {	
	var html = '<h3 id="pane_title" style="margin-bottom:10px">Task History</h3>';
...
function getTaskHistoryAsHTML() { 
	...
	var taskHistory = g_taskHistories[task-1];
	// old on top
...
function getStudentHistoryAsHTML(studentNickname) { 
	...
	if (taskHistory.length == 0) {
		...
		for (var i=taskHistory.length-1; i>=0; i--) {
			...
			var taskType = taskItem.activity_type;	
			html += '<tr>';
			html += '<td style="width:17ex">' + '<div style="width:'+ACTION_DIM+'px;height:'+ACTION_DIM+'px !important;background:'+ACTION_COLORS[taskType]+'; float:left; margin-right:4px; margin-top:7px;">&nbsp;</div> ' + type.replace(" ", "&nbsp;") + '</td>';
			html += '<td>' + details + '</td>';
			html += '<td style="width:15ex">' + getFormattedTimestamp(taskTime) + '</td>';
			html += '</tr>';
		}
...
function drawStudentHistory(div, studentNickname) {
	...
	var numTasksDrawn = 0;
 	for (var i=0; i<taskHistory.length; i++) {
 		...
 		var title = '';
 		actionHtml += '<div id="event_"'+(i+1)+' title="'+title+'" style="width:'+ACTION_DIM+'px;height:'+ACTION_DIM+'px !important;background:'+ACTION_COLORS[type]+';float:left;margin-right:'+actionMargin+'px;margin-top:'+topMargin+'px;">&nbsp;</div>';
...
AccordionList.prototype.show = function() {
	...
	if (this.items.length==0) {
		...
		$('#accordion_list').accordion({
			collapsible: true, 
			active: this.getSelectedIndex(this.selectedKey),
			change: function(event, control) {
				var selectedItem = $(".accordion_section:eq("+control.options.active+")");
				this.selectedKey = $('.text_key', selectedItem).html();
			}
		});
	}
}
	...
	for (var taskIdx=0; taskIdx<g_lessons[0].tasks.length; taskIdx++) {
		...
		for (var studentNickname in g_students) {
			...
			for (var i=0; i<student.task_history[taskIdx].length; i++) {
				...
				g_taskHistories[taskIdx].push(task);
			}
		}
		
		g_taskHistories[taskIdx].sort(function(x, y) {
